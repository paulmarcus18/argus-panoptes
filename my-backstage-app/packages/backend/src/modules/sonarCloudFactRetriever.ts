  ...
  return {
    ...
    handler: async ctx => {
      ...
      const sonarcloudEntities = entities.filter(entity => 
        ...
        entity.metadata.annotations?.['sonarcloud.io/project-key'] && // && (... xor ... xor ...) if each repo is part of a different system
        entity.spec?.system === 'payments-system'
      );
      
      // if (sonarcloudEntities.length === 0) {
      //   logger.info('No entities with SonarCloud enabled found');
      //   return [];
      // }
