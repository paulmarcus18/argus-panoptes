...
> = ({ open, onClose, entities = [] }) => {
  ...
  useEffect(() => {
    ...
    const fetchSystemConfiguration = async () => {
      ...
      if (!systemName) {
        return { redThreshold: 0.33, configuredRepoNames: [] };
      }
      ...
      return { redThreshold, configuredRepoNames };
    };
  
    const fetchMetrics = async () => {
      // 1. Fetch system threshold and configured repositories
      const systemConfig = await fetchSystemConfiguration().catch(() => ({
        redThreshold: 0.33,
        configuredRepoNames: [] as string[],
      }));
  
      // 2. Filter entities to only include configured repositories
      const filteredEntities =
        systemConfig.configuredRepoNames.length > 0
          ? entities.filter(entity =>
              systemConfig.configuredRepoNames.includes(entity.metadata.name),
            )
      ...
      const results = await Promise.all(
        filteredEntities.map(async entity => {
          ...
          const successRate =
            facts.successWorkflowRunsCount + facts.failureWorkflowRunsCount > 0
              ? (facts.successWorkflowRunsCount /
                  (facts.successWorkflowRunsCount + facts.failureWorkflowRunsCount)) * 100
              : 0;
      ...
      if (systemConfig.configuredRepoNames.length > 0) {
