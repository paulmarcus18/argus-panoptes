...
describe('determineSemaphoreColor', () => {
  ...
  it('returns red when failures exceed red threshold', () => {
    ...
    expect(result.reason).toMatch(/3 out of 5 entities failed.*threshold of 50.0%/);
  });
  ...
  it('returns yellow when failures are within threshold', () => {
    ...
    expect(result.reason).toMatch(/2 out of 5 entities failed.*threshold of 50.0%/);
  });
  ...
  it('returns red when a single failure exceeds threshold with 1 total entity', () => {
    ...
    expect(result.color).toBe('red');
    expect(result.reason).toMatch(/1 out of 1 entity failed.*threshold of 50.0%/);
  });
