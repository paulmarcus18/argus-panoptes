...
import { aiSummaryBackendPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [aiSummaryBackendPlugin],
    });

    await request(server).get('/api/ai-summary-backend/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/ai-summary-backend/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/ai-summary-backend/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/ai-summary-backend/todos/${createdTodoItem.id}`)
