...
import { ArgusPanoptesRuleBooleanCheckResult, ArgusPanoptesRuleCheck } from '../types';
 ...
 *
 * Constructor options for ArgusPanoptesRulesEngineFactChecker
 */
export type ArguspanoptesRulesEngineFactCheckerOptions = {
  checks: ArgusPanoptesRuleCheck[];
 ...
 */
export class ArgusPanoptesRulesEngineFactChecker
  implements FactChecker<ArgusPanoptesRuleCheck, ArgusPanoptesRuleBooleanCheckResult>
{
  private readonly checkRegistry: TechInsightCheckRegistry<ArgusPanoptesRuleCheck>;
  private repository: TechInsightsStore;
  private readonly logger: LoggerService;
  private readonly validationSchema: SchemaObject;
  private readonly operators: Operator[];

  constructor(options: ArguspanoptesRulesEngineFactCheckerOptions) {
    ...
    this.checkRegistry =
      checkRegistry ??
      new DefaultCheckRegistry<ArgusPanoptesRuleCheck>(checks);
  }

  async runChecks(
    entity: string,
    checks?: string[],
  ): Promise<ArgusPanoptesRuleBooleanCheckResult[]> {
  ...
  getChecks(): Promise<ArgusPanoptesRuleCheck[]> {
  ...
  private async ruleEngineResultsToCheckResponse(
    ...
    techInsightChecks: ArgusPanoptesRuleCheck[],
    ...
    return await Promise.all(
      [
        ...
        return {
          ...
          check: ArgusPanoptesRulesEngineFactChecker.constructCheckResponse(
            techInsightCheck,
            result,
          ),
        };
  ...
  private static constructCheckResponse(
    techInsightCheck: ArgusPanoptesRuleCheck,
    result: any,
  ) {
    const returnable = {
 ...
 * @public
 *
 * Constructor options for ArgusPanoptesRulesEngineFactCheckerFactory
 *
 ...
 */
export type ArgusPanoptesRulesEngineFactCheckerFactoryOptions = {
  checks: ArgusPanoptesRuleCheck[];
  logger: LoggerService;
  checkRegistry?: TechInsightCheckRegistry<ArgusPanoptesRuleCheck>;
  operators?: Operator[];
};
 ...
 * @public
 *
 * Factory to construct ArgusPanoptesRulesEngineFactChecker
 ...
 */
export class ArgusPanoptesRulesEngineFactCheckerFactory {
  private readonly checks: ArgusPanoptesRuleCheck[];
  private readonly logger: LoggerService;
  private readonly checkRegistry?: TechInsightCheckRegistry<ArgusPanoptesRuleCheck>;
  private readonly operators?: Operator[];
  ...
  static fromConfig(
    config: Config,
    options: Omit<ArgusPanoptesRulesEngineFactCheckerFactoryOptions, 'checks'>,
  ): ArgusPanoptesRulesEngineFactCheckerFactory {
    const checks = readChecksFromConfig(config, { logger: options.logger });

    return new ArgusPanoptesRulesEngineFactCheckerFactory({
      ...options,
      checks,
    });
  }

  constructor(options: ArgusPanoptesRulesEngineFactCheckerFactoryOptions) {
  ...
  /**
   * @param repository - Implementation of TechInsightsStore. Used by the returned ArgusPanoptesRulesEngineFactChecker
   *                     to retrieve fact and fact schema data
   * @returns ArgucPanoptesRulesEngineFactChecker implementation
   */
  construct(repository: TechInsightsStore) {
    return new ArgusPanoptesRulesEngineFactChecker({
      checks: this.checks,
      logger: this.logger,
